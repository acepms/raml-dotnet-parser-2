    ...
    amf.AMF.init()
        .then(function () {
            ...
            parser.parseFileAsync(url).then(function (amfModel) {
                var declares = amfModel.declares; // this has all the types, securitySchemes, traits, resourceTypes, annotations, etc
                let resolvedModel = amf.AMF.resolveRaml10(amfModel);
...
function mapDocumentations(documentations){
    if(!documentations)
        return [];
...
function mapSecurities(securities){
    if(!securities)
        return [];
...
function mapSecuritySchemes(securitySchemes){
    if(!securitySchemes)
        return [];
...
function mapSettings(settings, type){
    if(!settings)
        return null;
